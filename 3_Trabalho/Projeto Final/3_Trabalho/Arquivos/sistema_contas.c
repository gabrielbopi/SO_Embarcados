...
int main(int argc, char * argv[]){
	...
	if(argv[1][0]=='-'){
		...
		return -1;
				...
				resposta = adicionaUsuario(arquivo_dados,entrada_usuario,entrada_senha, atoi(entrada_creditos));
...
//
		switch(pedido){
			...
			break;
			case 'm':
				resposta = adicionaUsuario(arquivo_dados,usuario_solicitado.nome,usuario_solicitado.senha, usuario_solicitado.creditos);
				if (resposta != 0) {printf("Erro no cadastro!\n");}
				//printf("ID: %d\n", usuario_corrente.id);
			break;
			defaut:
